...
def checkpoint(fn, *args):
    if loaded_options is None:
        ...
        enabled = (
            loaded_options["checkpointing_enabled"]
            if "checkpointing_enabled" in loaded_options.keys()
            else True
        )
    if enabled:
...
def clip_grad_norm(
    parameters: list, parameter_names: list, max_norm: float, norm_type: float = 2.0
) -> torch.Tensor:
    r"""
...
def load_model_from_config(
    cfg_file=None,
    model_name=None,
    also_load_savepoint=True,
    load_path=None,
    preloaded_options=None,
    strict_load=True,
    device=None,
):
